...
struct LoadedScript loaded_scripts[MAX_LOADED_SCRIPTS];
...
int load_script_to_backing_store(char *script, int file_number);
int get_total_lines(FILE *file_ptr);
static bool load_page_into_frame(FILE *file, struct PCB *pcb, int page_num, int frame_num);
static int find_loaded_script(char *script);
...
int interpreter(char* command_args[], int args_size) {
    ...
    if (strcmp(command_args[0], "help") == 0){
        ...
        load_script_to_backing_store(command_args[1],1);
        ...
        //Load all the scripts into backing store
        //Skip the last argument which is the policy
        for(i = 1; i < args_size - 1; i++){
            load_script_to_backing_store(command_args[i],i);
        }
...
int run(char* script) {
    ...
    // Open script from backing store with correct path
    char backing_store_path[MAX_USER_INPUT];
    snprintf(backing_store_path, sizeof(backing_store_path), "backing_store/1");  // Use file number 1 for run command
    
    FILE *backing_store = fopen(backing_store_path, "rt");
    if (!backing_store) {
        return badcommandFileDoesNotExist();
    }

    // Rest of the function remains the same
    int total_lines = get_total_lines(backing_store);
    struct PCB pcb = createPCB(backing_store_path, total_lines, 0);
    
    // Initialize LRU queue
    struct LRUQueue lru_queue;
    if (!init_lru_queue(&lru_queue)) {
    ...
    for (int page_num = 0; page_num < INITIAL_PAGES && !loaded_all; page_num++) {
        int frame_num = getNextAvailablePage();
        if (frame_num == -1) {
            break;
        }

        loaded_all = load_page_into_frame(backing_store, &pcb, page_num, frame_num);
        pcb.pagetable[page_num] = frame_num;
...
int exec(char* command_args[], int args_size) {
    ...
    */
        ...
        for (int i = 1; i < args_size - 1; i++) {
            ...
            int existing_file = find_loaded_script(command_args[i]);
            if (existing_file != -1) {
                file_num = existing_file;
            
            } else {
                file_num = i;
                load_script_to_backing_store(command_args[i], file_num);
            }

            snprintf(backing_store_path, sizeof(backing_store_path), "backing_store/%d", file_num);
            FILE *backing_store = fopen(backing_store_path, "rt");
...
/**
 * Comparator function for sorting PCBs by length
 */
int pcb_length_comparator(const void *a, const void *b) {
...
/**
 * A3-2024
 * Loads a script into the backing store
 */
int load_script_to_backing_store(char *script, int file_number) {
    // Check if script already loaded
    int existing_file = find_loaded_script(script);
    if (existing_file != -1) {
        return 0; // Script already in backing store
    }
    ...
    // Create new file in backing store
    char backing_store_path[MAX_USER_INPUT];
    snprintf(backing_store_path, sizeof(backing_store_path), "backing_store/%d", file_number);
    FILE *backing_store_file = fopen(backing_store_path, "w");
    ...
    // Add to loaded scripts table
    if (num_loaded_scripts < MAX_LOADED_SCRIPTS) {
        loaded_scripts[num_loaded_scripts].name = strdup(script);
        loaded_scripts[num_loaded_scripts].file_number = file_number;
        loaded_scripts[num_loaded_scripts].is_loaded = true;
        num_loaded_scripts++;
    }
...
/**
 * A3-2024
 * Helper function to get the total number of lines in a file
 */
int get_total_lines(FILE *file_ptr) {
...
/**
 * A3-2024
 * Helper function to load a page into a frame
 */
static bool load_page_into_frame(FILE *file, struct PCB *pcb, int page_num, int frame_num) {
    char line[MAX_USER_INPUT];
    ...
    for (int i = 0; i < PAGE_SIZE; i++) {
        ...
        mem_load_line(PAGE_SIZE * frame_num + i, pcb->scriptName, line);
    }
...
/**
 * A3-2024
 * Helper function to find a loaded script in the loaded_scripts array
 */
static int find_loaded_script(char *script) {
    for (int i = 0; i < num_loaded_scripts; i++) {
        if (loaded_scripts[i].is_loaded && strcmp(loaded_scripts[i].name, script) == 0) {
            return loaded_scripts[i].file_number;
        }
    }
    return -1;
}